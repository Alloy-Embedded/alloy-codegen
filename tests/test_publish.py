...
def test_publish_blocks_when_runtime_capability_sidecar_is_missing(
    execution_context: ExecutionContext,
    monkeypatch,
) -> None:
    emit_result = run_emit(PipelineScope(device="stm32g071rb"), execution_context)
    missing_path = "st/stm32g0/generated/runtime/devices/stm32g071rb/capabilities.json"
    filtered_artifacts = tuple(
        artifact for artifact in emit_result.payload.artifacts if artifact.path != missing_path
    )
...
def test_publish_blocks_when_capability_regression_is_detected(
    execution_context: ExecutionContext,
    monkeypatch,
) -> None:
    scope = PipelineScope(device="stm32g071rb")
    ...
    published_payload["capabilities"].append(
        {
            ...
            "name": "synthetic-regression-probe",
            "value": "true",
            "peripheral": None,
        }
    )
    ...
    assert result.payload.publication_mode == "blocked"
    assert result.warnings
    assert "capability regression was detected" in result.warnings[0]
    assert "stm32g071rb removed capability ids" in result.warnings[0]
    assert removed_capability_id in result.warnings[0]
    assert (
        execution_context.publication_root
        / "st"
        / "stm32g0"
...
def test_publish_preserves_git_checkout_metadata_and_unrelated_files(
    ...
    # The auto-generated README is now emitted as a publication artifact
    # (`add-publication-scale-features`).  The previous test asserted that the
    # publish stage *preserved* a manually-crafted README — that contract is
    # superseded: the publish stage now OVERWRITES the README with the
    # canonical auto-generated content.  A manual edit visible in `git status`
    # is the expected signal that something changed.
    regenerated = readme_path.read_text(encoding="utf-8")
    assert regenerated.startswith("# alloy-devices"), (
        "publish must regenerate the README from runtime_readme.emit_devices_readme"
    )
    assert "Auto-generated by alloy-codegen" in regenerated
    assert "## Admitted families" in regenerated
    ...
    # README.md is materially different from the bootstrap stub, so git
    # status reports it as modified.
    assert "M README.md" in status.stdout or " M README.md" in status.stdout
