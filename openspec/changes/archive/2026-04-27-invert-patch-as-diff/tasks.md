...
- [x] 1.1 Extract a "source-derived baseline IR" builder from the
...
- [x] 1.2 Document the contract: a baseline has every field the
      patch can override, populated from authoritative sources.

## Phase 2: Patch-as-diff loader

- [x] 2.1 New loader `patches.load_patch_as_diff(...)` that
      deep-merges a patch JSON on top of a baseline IR.
- [x] 2.2 Add `"$baseline-revision"` schema field at the top of
      every patch — records the source SHA the diff was computed
      against.
- [x] 2.3 Validation: refuse to load a patch whose
...
- [x] 3.1 `scripts/minify_device_patches.py` walks every existing
      patch, computes diff vs. baseline, rewrites in place.
- [x] 3.2 Reversible `--explode` mode re-merges baseline + diff
      to produce the full patch (for human inspection).
- [x] 3.3 Validation gate: a patch SHALL NOT contain any field
      that matches the baseline value — checked by a unit test.

## Phase 4: Per-family migration

- [x] 4.1 Migrate STM32G0 first (smallest fixture).  Run
      minifier, verify pipeline output unchanged, commit reduced
      patches.
- [x] 4.2 Repeat for the remaining 8 admitted families.
- [x] 4.3 Goldens unchanged after migration — patches shrink, but
      emitted artifacts are byte-identical.

## Phase 5: Spec + final checks

- [x] 5.1 Spec delta in `specs/canonical-device-ir/spec.md`.
- [x] 5.2 Document the diff model in `docs/patch-as-diff.md`.
- [x] 5.3 `openspec validate invert-patch-as-diff --strict` passes.
- [x] 5.4 `pytest -q` + `ruff check` clean.
