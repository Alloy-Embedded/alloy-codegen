...
- [x] A.1 New IR types `StmTimerPwmDescriptor` (with `kind`,
...
- [x] A.2 JSON schema (`schemas/canonical-device-ir-v1.schema.json`)
      and `connector_model.py` carry-forward.
- [x] A.3 `_build_st_timer_pwm_peripherals` in
...
- [x] B.1 Extend `LedcDescriptor` with `timer_count`,
      ...
      fields.
- [x] B.2 New `McpwmDescriptor`. ESP32 classic and S3 each ship two
...
- [x] B.3 Emitter: extend the existing LEDC trait emission and add
...
- [x] B.4 Tests: LEDC field expansion (kMaxResolutionBits=20 on
...
- [x] C.1 New `FlexPwmDescriptor` with `submodule_count = 4`,
...
- [x] C.2 Wire `_build_nxp_flex_pwm_peripherals` from the iMXRT
      normalize path; the IOMUX entries already flow through pins,
      so this mirrors the STM32 helper's filter pattern.
- [x] C.3 Emitter: `_flex_pwm_traits_block` (4 specializations per
      device — PWM1..PWM4).
- [x] C.4 Tests: PWM1 SubModule0 A/B pin sets on mimxrt1062.

## Phase D — AVR-DA TCA + SAME70 PWM/TC

- [x] D.1 New `AvrDaTcaPwmDescriptor` with `default_channel_pins`,
      ...
      follow-up.
- [x] D.2 New `Same70PwmDescriptor` with `kind` (`"pwm"` /
      `"tc"`), `channel_count`, `valid_pins_per_channel`,
      `supports_*` flags. Wire into `_build_microchip_device_ir`.
- [x] D.3 Emitter: `_avr_da_tca_pwm_traits_block` and
      `_same70_pwm_traits_block`.
- [x] D.4 Tests: TCA0 default placement; PWM0 channel set on SAME70.

## Phase E — Compile tests + per-family coverage gate

- [x] E.1 New compile-time `static_assert` smokes:
...
- [x] E.2 New `tests/test_pwm_semantic_coverage.py` with one test
...
- [x] F.1 Update `docs/COVERAGE_MATRIX.md` with a `pwm_traits` column
...
- [x] F.2 `openspec archive extend-pwm-coverage-all-mcus`.
