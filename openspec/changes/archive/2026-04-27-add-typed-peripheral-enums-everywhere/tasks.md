...
> **Implementation note (Phase 7+):** the publication gate forbids
> string literals in runtime C++ output (firmware-image bloat).  The
> name-table tasks below were planned but **not shipped** —
> consumers needing stringification implement it host-side via a
> switch over the typed enum.  The typed enums themselves are the
> value-add; the spec was updated to reflect this.

## Phase 1: Shared typed-enum renderer

- [x] 1.1 Lift the ADC channel-enum renderer (added by
...
- [x] 1.2 Refactor the ADC emitter to use the new shared renderer
      (no behaviour change; goldens stay byte-stable).

## Phase 2: UART typed enums

- [x] 2.1 Emit `UartParityOf<P>::type` with entries `none`, `even`,
      `odd`, `mark`, `space` filtered by the peripheral's
      `parity_options`.
- [x] 2.2 Emit `UartStopBitsOf<P>::type` with entries `half`,
      `one`, `one_and_half`, `two` keyed by `stop_bits_q8`.
- [x] 2.3 Emit `UartOversamplingOf<P>::type` with `over_8x`,
      `over_16x`.
- [x] 2.4 Emit `UartBaudClockSourceOf<P>::type` with entries
      derived from the source-classifier (`pclk`, `sysclk`,
      `hsi16`, `lse`, `xtal`, `apb`, `peri_clk`, `clk_per`,
      `lpuart_clk_root`, `rc_fast`, `ref_tick`).
- [x] 2.5 Emit `UartDataBitsOf<P>::type` with named entries `bits_5`
      … `bits_10`.
- [x] 2.6 Emit `UartFifoTriggerOf<P>::type` with named entries for
      the populated `fraction_q8` values (`one_eighth`, `quarter`,
      `half`, `three_quarters`, `seven_eighths`, `full`).
- [x] 2.7 Pair every typed enum with a typed `std::array<...Of<P>::type, N>`
      constexpr alongside the existing raw `_Raw` arrays.  Keep the
      raw arrays one cycle for back-compat.

## Phase 3: SPI typed enums

- [x] 3.1 `SpiPrescalerOf<P>::type` with entries `div_2`, `div_4`,
      …, `div_256` filtered by populated divisors.
- [x] 3.2 `SpiFrameSizeOf<P>::type` with `bits_4` … `bits_32` filtered
      by populated frame sizes.
- [x] 3.3 `SpiFifoThresholdOf<P>::type` with the populated
      `threshold_bits` named entries.

## Phase 4: I2C typed enums

- [x] 4.1 `I2cSpeedModeOf<P>::type` with `standard`, `fast`,
      `fast_plus`, `high_speed` filtered by populated speed_options.

## Phase 5: TIMER typed enums

- [x] 5.1 `TimerTriggerSourceOf<P>::type` with the union of source
      strings populated in `timer_trigger_sources` (`itr0`..`itr3`,
      `etr`, `ti1f_ed`, etc.).
- [x] 5.2 `TimerMasterOutputOf<P>::type` with the union of master-output
      sources (`reset`, `enable`, `update`, `compare_pulse`,
      `oc1ref`..`oc4ref`).

## Phase 6: PWM typed enums

- [x] 6.1 `PwmAlignmentOf<P>::type` with `edge`, `center_up`,
      `center_down`, `center_up_down` filtered by populated
      alignment_options.
- [x] 6.2 `PwmBreakInputOf<P>::type` with the union of break-input
      IDs (`bkin`, `bkin2`, `fault0`..`fault3`).

## Phase 7: Tests + goldens

- [x] 7.1 Per-peripheral regression tests asserting the typed enum
      exists and contains the expected named entries on a populated
      device (USART1 on STM32G0, SPI1, I2C1, TIM1).
- [x] 7.2 Compile-time round-trip test: `to_string(parity::even) ==
      "even"`.
- [x] 7.3 Negative test: applying a peripheral that doesn't support
...
- [x] 7.4 Regenerate emit-fixture goldens.  Diff scope: ~20 new
      typed enum blocks per device with admitted Tier 2/3/4 data
      (UART/SPI/I2C/TIMER/PWM coverage).

## Phase 8: Spec + final checks

- [x] 8.1 Spec delta in `specs/artifact-contract/spec.md`.
- [x] 8.2 `openspec validate add-typed-peripheral-enums-everywhere
      --strict` passes.
- [x] 8.3 Full `pytest -q` + `ruff check` clean.
- [x] 8.4 Archive entry notes that this closes the ergonomics gap
...
- [x] 9.1 Add a docs/migration note that the `_Raw` `std::uint8_t`
