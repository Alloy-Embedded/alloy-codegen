...
## 1. IR vocabulary
- [x] 1.1 Add `MulticoreTopology` enum to `src/alloy_codegen/ir/model.py`
      with values `single_core`, `symmetric_dual_core`,
      `xtensa_asymmetric_dual_core`. Default factory returns `single_core`.
- [x] 1.2 Add `AppCpuControlPlane` dataclass with fields:
...
- [x] 1.3 Add `Device.multicore_topology: MulticoreTopology = single_core`
      ...
      cycle.
- [x] 1.4 Add `RegisterDescriptor.role: RegisterRole = general` with
      `RegisterRole = Literal["general", "secondary-core-release"]`.
- [x] 1.5 Update IR JSON serializers and JSON Schema to round-trip the new
      fields. Existing fixtures must still validate (defaults preserve
      back-compat).
- [x] 1.6 Unit tests in `tests/test_ir_model.py` covering construction,
      defaults, JSON round-trip.

## 2. Source-data overlays
- [x] 2.1 `patches/espressif/esp32/family.json` gains a `multicore_topology`
...
- [x] 2.2 `patches/espressif/esp32s3/family.json` gains the analogous LX7
      block with both `SYSTEM.CORE_1_CONTROL_0` and
      `SYSTEM.CORE_1_CONTROL_1` and `operation =
      "clear-runstall-after-clkgate"`.
- [x] 2.3 Patch validator asserts: asymmetric Xtensa devices MUST have a
      populated `multicore_topology` block; symmetric or single-core devices
      MUST NOT.
- [x] 2.4 `esp32c3` patch is unchanged (single-core RISC-V).

## 3. Normalizer wiring
- [x] 3.1 `src/alloy_codegen/sources/esp_idf.py` (or the relevant SVD adapter)
      reads the patch's `multicore_topology` and populates
      `Device.multicore_topology` + `Device.app_cpu_control_plane`.
- [x] 3.2 The normalizer resolves the patch's `register = "DPORT.APPCPU_CTRL_B"`
...
- [x] 4.3 `runtime_capabilities.py` writes:
...
- [x] 4.4 `runtime_xtensa_startup.py` is refactored to read addresses
...
## 5. Goldens & tests
- [x] 5.1 Regenerate `tests/fixtures/emitted/esp32/`,
...
- [x] 5.4 New positive test for esp32s3 covering both registers.
- [x] 5.5 New negative test:
      `test_esp32c3_has_no_secondary_core_release_step`.
- [x] 5.6 New IR test asserting `Device.multicore_topology` matches the
      patch overlay for all three Espressif devices.

## 6. Spec validation
- [x] 6.1 `openspec validate expose-xtensa-dual-core-facts --strict` passes.
- [x] 6.2 Full pytest suite passes (`pytest -q`).
- [x] 6.3 Diff review: confirm no goldens for non-Espressif devices changed
      beyond defaulting `multicore-topology = "single-core"` /
      `core-count = 1`.

## 7. Downstream unblock
- [x] 7.1 Ping the alloy `add-esp32-classic-family` change owner: phase 4
      ...
      release step.
- [x] 7.2 Note in the archive entry that this change unblocks alloy's
