...
def test_no_admitted_device_artifact_exceeds_warn_budget(
    ...
    family_to_vendor = {
        family: vendor for (vendor, family) in DEVICE_REGISTRY
    }
    ...
    for family, arts in grouped.items():
        ...
        violations.extend(
            evaluate_artifacts(
                artifacts=tuple(arts), vendor=vendor, family=family
            )
        )
...
def test_evaluate_uses_content_bytes_when_provided() -> None:
    ...
    violations = evaluate_artifacts(
        artifacts=(artifact,), vendor="st", family="stm32g0"
    )
    assert violations == ()
...
def test_violation_sort_order_lists_fails_before_warns() -> None:
    ...
    violations = evaluate_artifacts(
        artifacts=(warn_art, fail_art), vendor="st", family="stm32g0"
    )
